        ...
        do {
            return try backgroundContext.performAndWait {
                ...
                for entry in entries {
                    let userMessage = Message(id: entry.id!, rootId: entry.id!, inReplyToId: nil, createdAt: Date(), text: entry.copiedText!, isCurrentUser: true, isHidden: false, appContext: appContext)
                    let assistantMessage = Message(id: entry.id!, rootId: entry.id!, inReplyToId: nil, createdAt: Date(), text: entry.pastedResponse!, isCurrentUser: false, isHidden: false, appContext: appContext)
